...
public class Prim {
    ...
    public Edge selectMinimumWeight(List<Edge> edges, ArrayList<Vertex> visited) {
    	// Arc par d�faut, on lui donne une valeur infinie
        Edge selected = new Edge(new Vertex(0), new Vertex(0), Integer.MAX_VALUE);
        
        // On regarde tous les arcs du graph
    	for(Edge e : edges) {
    		// Si un arc est a un sommet A visit� et B non visit� et a un poids
    		// plus faible que l'arc en cours, on le s�lectionne
    		if(e.getWeight() < selected.getWeight() && visited.contains(e.getFrom()) && !visited.contains(e.getTo()))
    			selected = e;
    	}
    	
    	// S'il n'a pas trouv� d'arc, on ne veut pas r�cup�rer l'arc par d�faut
